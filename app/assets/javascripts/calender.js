$(document).ready(function(){
  ...
  var calendar = $('#calendar').fullCalendar({
    ...
    weekends: true,                            //土曜日、日曜日を表示
    weekMode: 'fixed',                         //週モード（fied, liquid, variable）
    ...
    eventClick: function(event) {  //イベントをクリックした時に実行
      var id = event.id
    ...
    eventResize: function(event) {　//イベントをサイズ変更した際に実行
      ...
      var hour = ( event_end_time.getDate() < 10 ) ? '0' + event_end_time.getHours() : event_end_time_getHours();
      var min = ( event_end_time.getMinutes() < 10 ) ? '0' + event_end_time.getMinutes() : event_end_time.getMinutes();
      var momemnt_end = year+"-"+month+"-"+day+" "+hour+":"+min;
      var end_time = moment(moment_end).add(-9, 'hour').format("YYY-MM-DD HH:mm");
      var data = {
        event: {
          title: event.title,
          start: start_time,
